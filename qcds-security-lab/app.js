...
function comparisonReadout() {
  ...
  const fact =
    ...
    m.dimensions.find((d) => d.weakened?.length || d.lost.length) ||
    m.dimensions[0];
  const routeCount = allPaths().length;
  return `<div class="comparison-readout"><div><span>CHANGE THE VIEW</span><h3>Remove a perspective</h3><p>${lens ? `Without <b>${esc(lens.name)}</b>, ${lens.retained.length} of ${routeCount} surviving routes remain visible through the other lenses.` : "Enable a perspective and run the analysis to compare views."}</p><small>Question: does this route space depend on one lens?</small></div><div><span>CHANGE ONE INPUT</span><h3>Turn one 1 into ?</h3><p>${fact ? `With <b>${fact.id} · ${esc(FIELD_META[fact.key][0])}</b> treated as ?, ${fact.weakened?.length ? fact.weakened.join(", ") + " become more conditional" : "no route becomes more conditional"}${fact.lost.length ? "; " + fact.lost.join(", ") + " disappear" : ""}.` : "Declare a Yes condition to compare its contribution."}</p><small>Question: does the route depend on this dimension?</small></div></div>`;
}
...
function metrics() {
  const m = state.model;
  return `<div class="metrics"><div><span>Active routes</span><strong>${m.findings.length}<small>all required facts = 1</small></strong></div><div><span>Conditional routes</span><strong class="${m.pending.length ? "amber" : ""}">${m.pending.length}<small>kept alive by ?</small></strong></div><div><span>Conditions unknown</span><strong class="${m.unknown.length ? "amber" : ""}">${m.unknown.length}<small>QCDS asks next</small></strong></div><div><span>Evidence attached</span><strong>${m.findings.filter((f) => f.records.length).length}<small>of ${m.findings.length} active routes</small></strong></div></div>`;
}
function graph() {
...
function reportView() {
  ...
  return (
    ...
    <h3>QCDS search-space summary</h3><ul><li><b>${m.searchSpace.confirmedRoutes}</b> active routes have all required facts present.</li><li><b>${m.searchSpace.conditionalRoutes}</b> conditional routes remain alive because required facts are still ?.</li><li><b>${m.searchSpace.rejectedRoutes}</b> seed routes are rejected by current declared conditions or lens applicability.</li><li><b>${m.unknown.length}</b> conditions remain unresolved.</li><li><b>${m.findings.filter((f) => f.records.length).length}</b> active routes have user-reported evidence.</li></ul>
    ...
    <h3>Scope of the conclusion</h3><p class="small">This browser implementation performs ternary condition formation, conditional route retention, perspective reruns, dimension comparisons, recursive control challenge and evidence binding over eight current seed route families. It does not autonomously scan the target, execute a quantum circuit or independently certify a vulnerability. The seed families initialize the browser search space; they are not the limit of the QCDS architecture.</p>
