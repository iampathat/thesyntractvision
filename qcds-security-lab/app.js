...
function perspectiveMarkdown(name = state.perspective) {
  ...
  return [
    ...
    `QCDS mask: ${m.searchSpace.knownMaskDimensions} fixed 1 / 0 dimensions + ${m.searchSpace.unknownMaskDimensions} unresolved ? dimensions → ${m.searchSpace.maskedLogicalSpace}${m.searchSpace.maskedLogicalStates !== null ? " = " + m.searchSpace.maskedLogicalStates.toLocaleString("en-US") : ""} logical mask states.`,
  ...
  return (
    ...
    `<section class="vector-fabric-summary panel">
      ...
      <div class="vector-fabric-metrics">
        ...
        <div><span>MASK SPACE</span><b>${m.searchSpace.maskedLogicalSpace}</b><small>${m.searchSpace.unknownMaskDimensions} unresolved ? dimension${m.searchSpace.unknownMaskDimensions === 1 ? "" : "s"}</small></div>
