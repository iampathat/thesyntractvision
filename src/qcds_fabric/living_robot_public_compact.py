...
_CSS = r'''
.publicBuildMark{font-size:7px;letter-spacing:.14em;text-transform:uppercase;color:#91aabb;border:1px solid #29495d;background:#091a27;border-radius:999px;padding:5px 7px;white-space:nowrap}.publicCompactBar{max-width:1800px;margin:10px auto 0;padding:0 14px}.publicCompactInner{display:flex;align-items:center;gap:9px;flex-wrap:wrap;border:1px solid #31536b;background:linear-gradient(135deg,#091925,#0a2025);border-radius:13px;padding:9px 10px;box-shadow:0 10px 35px #0003}.publicCompactLead{flex:1;min-width:260px}.publicCompactLead b{display:block;font-size:9px;color:#e6f5fc}.publicCompactLead span{display:block;font-size:7.5px;line-height:1.4;color:#809dad;margin-top:2px}.publicCompactActions{display:flex;gap:6px;flex-wrap:wrap}.publicCompactActions button{padding:7px 9px;font-size:7px}.publicCapabilityStrip{max-width:1800px;margin:7px auto 0;padding:0 14px;display:flex;gap:6px;flex-wrap:wrap}.publicCapability{border:1px solid #27485b;background:#071722;border-radius:999px;padding:5px 8px;font-size:7px;color:#86a4b5}.publicCapability strong{color:#d9edf8}.publicCapability.quantum{border-color:#5c4f78;background:#100c1b;color:#ae9fc7}.publicCapability.swarm{border-color:#38634b;background:#071b13;color:#8eb49a}
body.publicCompact:not(.publicAdvancedOpen)>.hero,body.publicCompact:not(.publicAdvancedOpen)>.layout,body.publicCompact:not(.publicAdvancedOpen)>.learningMoment,body.publicCompact:not(.publicAdvancedOpen)>.understandBuild,body.publicCompact:not(.publicAdvancedOpen)>.domainLab,body.publicCompact:not(.publicAdvancedOpen)>.spaceBuilderWrap,body.publicCompact:not(.publicAdvancedOpen)>.sessionSandbox{display:none!important}body.publicCompact.publicAdvancedOpen>.hero{display:block!important}body.publicCompact.publicAdvancedOpen>.layout{display:grid!important}body.publicCompact.publicAdvancedOpen>.learningMoment,body.publicCompact.publicAdvancedOpen>.understandBuild,body.publicCompact.publicAdvancedOpen>.domainLab,body.publicCompact.publicAdvancedOpen>.spaceBuilderWrap,body.publicCompact.publicAdvancedOpen>.sessionSandbox{display:block!important}
...
<section class="publicCompactBar" id="public-overview">
  <div class="publicCompactInner">
    ...
    <div class="publicCompactLead"><b>ONE QCDS · MULTIPLE ORACLE MANIFESTATIONS</b><span>Start with a quick Logical Space or the Swedish Legal Robot. Older explanatory surfaces are folded away by default.</span></div>
    <div class="publicCompactActions">
      <button type="button" onclick="publicGo('try-logical-robot')">TRY QCDS</button>
      <button type="button" onclick="publicGo('swedish-legal-robot')">LEGAL ROBOT</button>
      ...
      <button type="button" onclick="publicToggleAdvanced()">ADVANCED</button>
    </div>
  </div>
</section>
...
_SCRIPT = r'''
<script>
function publicGo(id){const el=document.getElementById(id);if(el)el.scrollIntoView({behavior:'smooth',block:'start'})}
function publicToggleAdvanced(){document.body.classList.toggle('publicAdvancedOpen')}
function publicToggleLegalDetails(){document.body.classList.toggle('publicLegalDetailsOpen');publicGo('swedish-legal-robot')}
function publicToggleCases(){document.body.classList.toggle('publicAllLegalCases');publicGo('swedish-legal-robot')}
...
def living_robot_public_compact_html(*, static_mode: bool = False) -> str:
    ...
    html = html.replace('<body>', '<body class="publicCompact">', 1)
