...
def create_live_robot_server(**kwargs: Any):
    """BUILD 37 presentation wrapper around the unchanged BUILD 35 runtime."""

    server = _BUILD35_CREATE_SERVER(**kwargs)
    service = getattr(server, "qcds_service")
    original_state = service.state

    def state_with_build37() -> dict[str, Any]:
    ...
    service.state = state_with_build37
    parent_handler = server.RequestHandlerClass

    class Handler(parent_handler):
        server_version = "QCDSLivingLogicalRobot/1.8"

        def do_GET(self) -> None:  # noqa: N802
            if urlparse(self.path).path != "/":
                super().do_GET()
                return
            body = living_robot_invite_html(static_mode=False).encode("utf-8")
