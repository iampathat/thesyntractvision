...
def _index_for_state(state: Sequence[int]) -> int:
    index = 0
    for value in state:
        index = (index << 1) | int(value)
    return index


def _target_for_index(index: int, dimension_ids: tuple[str, ...]) -> dict[str, int]:
    return dict(zip(dimension_ids, _state_for_index(index, len(dimension_ids))))


def _matching_indices(view: ChannelView, state: State) -> tuple[int, ...]:
    """Resolve only the missing QCDS bits instead of scanning all 2^n states."""
    active = view.state_as_mapping(state)
    bits: list[int | None] = []
    for dimension_id in view.base_bundle.dimension_ids:
        bits.append(active.get(dimension_id))
    unknown = [index for index, value in enumerate(bits) if value is None]
    if not unknown:
        return (_index_for_state(tuple(int(value) for value in bits)),)

    matches: list[int] = []
    for variant in range(1 << len(unknown)):
        resolved = list(bits)
        for offset, position in enumerate(unknown):
            resolved[position] = (variant >> offset) & 1
        matches.append(_index_for_state(tuple(int(value) for value in resolved)))
...
class ValidPositionOracle:
    ...
    def score(self, view: ChannelView, state: State) -> float:
        matches = _matching_indices(view, state)
...
class AdjacentDistributionOracle:
    """Carry the complete previous QCDS TruthDistribution into the next route pass."""
    ...
    def _score_index(self, index: int) -> float:
        ...
        return sum(
            ...
            for neighbor in _neighbors(_cell_from_index(index, self.width), self.width, self.height)
        )
...
def _view(bundle: BaseBundle, stack: OracleStack, fabric: FabricLayer) -> ChannelView:
...
def _probabilities(distribution: TruthDistribution, cell_count: int) -> dict[int, float]:
    out: dict[int, float] = {}
    for state, probability in zip(distribution.support, distribution.probabilities):
        if probability <= _EPSILON or any(value not in (0, 1) for value in state):
            continue
        index = _index_for_state(state)
        if index < cell_count:
            out[index] = float(probability)
    return out


def _frontier(distribution: TruthDistribution, width: int, height: int) -> list[tuple[int, int]]:
    return [_cell_from_index(index, width) for index in sorted(_probabilities(distribution, width * height))]
...
def _route_graph(layers: Sequence[Sequence[tuple[int, int]]], width: int, height: int):
    ...
    counts: dict[tuple[int, int], int] = {layers[0][0]: 1} if layers and layers[0] else {}
    for depth in range(1, len(layers)):
        ...
        for cell in layers[depth]:
            ...
            next_counts[cell] = sum(counts.get(parent, 0) for parent in candidates)
        parent_layers.append(parents)
        counts = next_counts
    return parent_layers, counts


def _representative_path(goal, start, parent_layers):
    if goal == start:
        return [start]
    current = goal
    path = [goal]
...
def run_robotics_route_qcds(
    ...
    *,
    ...
    start = _cell(payload.get("start", (1, 6)), name="start")
    goal = _cell(payload.get("goal", (18, 6)), name="goal")
    ...
    bundle = BaseBundle(
        ...
        values=("?",) * bit_count,
        provenance={"source": "RoboticsPlayground", "cell_count": cell_count, "binary_position_encoding": True},
    ...
    blocked_oracles = tuple(
        BlockedPositionOracle(f"obstacle:{x}:{y}", _index((x, y), width))
        for x, y in sorted(blocked, key=lambda cell: (cell[1], cell[0]))
    )
    start_stack = OracleStack(
        "robotics-route-start",
        "1",
        (validity, *blocked_oracles, ExactOracle("position:start", _target_for_index(_index(start, width), dimension_ids))),
    )
    current = fabric_layer.kernel.run(_view(bundle, start_stack, fabric_layer), start_stack)
    layers = [_frontier(current, width, height)]
    distributions = [current]
    active_stack = start_stack
    reached = goal in layers[0]
    seen_frontiers = {frozenset(layers[0])}

    while not reached and len(layers) <= cell_count:
        transition = AdjacentDistributionOracle(
            f"transition:depth:{len(layers)}",
            width,
            height,
            _probabilities(current, cell_count),
        )
        active_stack = OracleStack(
            "robotics-route-recursive",
            f"1.{len(layers)}",
            (validity, *blocked_oracles, transition),
        )
        candidate = fabric_layer.kernel.run(_view(bundle, active_stack, fabric_layer), active_stack)
        if not candidate.raw_scores or max(candidate.raw_scores) <= _EPSILON:
            break
        frontier = _frontier(candidate, width, height)
        if not frontier:
            break
        current = candidate
        ...
        signature = frozenset(frontier)
        if not reached and signature in seen_frontiers:
            break
        seen_frontiers.add(signature)

    shortest_steps = len(layers) - 1 if reached else None
    parents, counts = _route_graph(layers, width, height)
    shortest_path_count = int(counts.get(goal, 0)) if reached else 0
    route = _representative_path(goal, start, parents) if reached else []

    syntract = None
    if reached:
        carried = DistributionOracle(
            "route:distribution-reentry",
            dimension_ids,
            {state: probability for state, probability in zip(current.support, current.probabilities)},
        )
        goal_oracle = ExactOracle("goal:reach", _target_for_index(_index(goal, width), dimension_ids))
        alignment_stack = OracleStack(
            "robotics-route-alignment",
            "1",
            (validity, *blocked_oracles, carried, goal_oracle),
        )
        ...
        aligned = suite.stabilized_return.stabilized_distribution
        syntract = Syntract(
            "syntract:robotics:route",
            aligned,
            {
                "source": "RoboticsPlayground",
                "recursive_depth": shortest_steps,
                "drawn_obstacle_oracles": len(blocked_oracles),
                "qcds_fabric_substrate": fabric_layer.substrate_id,
            },
            aligned.contradiction_markers,
            {
    ...
    return {
        ...
        "substrate_id": fabric_layer.substrate_id,
        "width": width,
        "height": height,
        "start": list(start),
        "goal": list(goal),
        ...
        "blocked": [list(cell) for cell in sorted(blocked, key=lambda cell: (cell[1], cell[0]))],
        "oracle_summary": {
        ...
        "qcds_phases": [
            {"number": 1, "name": "Condition Formation", "plain": f"Encode {cell_count} positions inside {bit_count} binary Conditions ({1 << bit_count} states)."},
            ...
            {"number": 4, "name": "Truth Alignment", "plain": "Re-enter the terminal distribution with the goal oracle and bind it as a Syntract without claiming the body route is unique."},
        ],
        "route_binding": {
            ...
            "syntract_id": syntract.syntract_id if syntract else None,
        },
        "quantum_explanation": (
            f"{bit_count} QCDS Conditions represent 2^{bit_count} candidate position states. Each recursive pass runs that bounded state space through the active oracle logic and re-enters the complete TruthDistribution. The browser uses the classical QCDS reference substrate; the logical contract is substrate-independent and this is not a quantum-speedup claim."
        ),
...
__all__ = ["run_robotics_route_qcds"]
