...
(() => {
  ...
  const MACHINE_LANGUAGES=[
    ...
    {code:'apple_icloud_calendar',name:'Apple iCloud Calendar',family:'vendor-caldav',generation:'modern',fidelity:'mapped',caps:['read','write','sync','caldav','apple-account-auth','app-password-fallback'],example:'iCloud CalDAV ↔ Calendar Space · Apple Account authorization / app-specific password'},
  ...
  function ensureState(){
    try{
      ...
      for(const def of defs){
        const i=state.dimension_states.findIndex(item=>item&&item.key===def.key);
        if(i<0){state.dimension_states.push(def);continue;}
        const current=state.dimension_states[i];
        let values=Array.isArray(current.values)&&current.values.length?current.values:def.values;
        if(def.key==='machine_language'){
          const currentByCode=new Map((Array.isArray(current.values)?current.values:[]).map(value=>[String(value?.code||''),value]));
          const canonicalCodes=new Set(def.values.map(value=>String(value.code)));
          values=[...def.values.map(value=>({...value,...(currentByCode.get(String(value.code))||{})})),...(Array.isArray(current.values)?current.values:[]).filter(value=>value?.code&&!canonicalCodes.has(String(value.code)))];
        }
        state.dimension_states[i]={...def,...current,values};
      }
