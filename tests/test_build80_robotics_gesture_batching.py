...
def test_robotics_batches_one_pointer_gesture_before_qcds_emulation() -> None:
    ...
    assert "if(Q75.editing||Q75.planning" in html


def test_robotics_does_not_emulate_once_per_drawn_cell() -> None:
    html = living_robot_public_html(static_mode=True)
    build80 = html.split("/* BUILD 80: one drawing gesture = one world edit = one QCDS re-inference. */", 1)[1]
