...
s=s.replace('<div class="destinationLabel">Samla till</div>','<div class="destinationLabel">Aktiv serie</div>',1)
s=s.replace('id="changeSeriesBtn" class="mini destinationChange" type="button">Ändra</button>','id="changeSeriesBtn" class="mini destinationChange" type="button">Byt serie</button>',1)
s=s.replace('<label class="chooserExisting"><span>Välj befintlig serie</span><select id="existingSeriesSelect"><option value="">Välj serie…</option></select></label>','<div class="chooserTitle">Välj serie</div><label class="chooserExisting"><select id="existingSeriesSelect"><option value="">Välj en befintlig serie…</option></select></label>',1)
s=s.replace('<summary>＋ Ny serie</summary>','<summary>＋ Skapa ny serie</summary>',1)
s=s.replace('<label><span>Serienamn</span><input id="seriesName" placeholder="T.ex. Plats A – foton"></label>','<label><span>Namn på serien</span><input id="seriesName" placeholder="T.ex. Plats A – foton"></label>',1)
s=s.replace('<label><span>Mapp <em>valfritt</em></span><input id="folderName" placeholder="Osorterat"></label>','<label><span>Mapp <em>valfritt, bara för ordning</em></span><input id="folderName" placeholder="Osorterat"></label>',1)
...
# Compact active series card: name first, folder and object count only.
pat=re.compile(r"el\.active\.innerHTML=`<strong>\$\{esc\(a\.name\)\}</strong><span class=\"small\">📁 \$\{esc\(a\.folder\|\|'Osorterat'\)\} · \$\{esc\(a\.category\|\|autoCategory\(a\.name\)\)\} · \$\{a\.itemIds\.length\} objekt</span>`")
repl="el.active.innerHTML=`<strong>${esc(a.name)}</strong><span class=\"small\">Mapp: ${esc(a.folder||'Osorterat')} · ${a.itemIds.length} objekt</span>`"
s,n=pat.subn(repl,s,count=1)
if n!=1:
    raise SystemExit('active series rendering pattern missing')

s=s.replace('Välj en serie med Ändra.','Tryck Byt serie för att välja eller skapa en serie.',1)

# Small heading and layout polish inside chooser.
...
s=s.replace('Detective Collector v0.21 · välj destination · samla · byt serie vid behov','Detective Collector v0.22 · serie först · tydligare lagringsstatus · mapp som sekundär ordning',1)
